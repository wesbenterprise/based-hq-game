...
const TILE = 16;
const SPEED = 140;
const PLAYER_W = 12; // half-width for collision
const PLAYER_H = 10; // half-height for collision

// ─── Room Bounds ─────────────────────────────────────────────────────────────
// Lobby: leftmost column, full height
const LOBBY = { x: 0, y: 0, w: 192, h: 768, name: 'lobby' };
// Hallway: horizontal corridor through the middle
const HALLWAY = { x: 192, y: 336, w: 1248, h: 96, name: 'hallway' };
// Top offices (above hallway, y=0-336)
const OFF_W = 312;
const OFF_H_TOP = 336;
const OFF_H_BOT = 336;
const OFFICES = {
...
// Door center X offsets (from office left edge, centered in office)
const DOOR_W = 48;
const DOOR_CX_OFFSET = 132; // 312/2 - 24 = 132 from left

// ─── Walkable Zones ───────────────────────────────────────────────────────────
function buildWalkable() {
  const zones = [];
  // Lobby interior (16px wall margin on all sides except right entrance)
  zones.push({ x: 16, y: 16, w: 160, h: 736, room: 'lobby' });
  // Lobby → Hallway connection
...
// ─── Room color palettes (Romero's Art Bible) ─────────────────────────────────
const ROOM_PALETTES = {
...
export default class HQScene extends Phaser.Scene {
  ...
  create() {
    ...
    // ── Player ─────────────────────────────────────────────────────────────
    this.playerSprite = this.physics.add.sprite(this.playerX, this.playerY, 'player');
  ...
  handleMovement(delta) {
    ...
    if (vx !== 0 || vy !== 0) {
      ...
      if (this.walkTimer > 120) {
        ...
        // Simple bob
        const bob = (this.walkFrame % 2 === 0) ? 0 : -2;
        this.playerSprite.setY(this.playerSprite.y + bob);
      }
    }
  }
  ...
  checkAgentProximity() {
    ...
    let closest = null;
    let closestDist = 72; // interaction radius
    ...
    if (closest) {
      ...
      // Camera space position for the hint
      const cam = this.cameras.main;
      const sx = (sprite.x - cam.scrollX) * cam.zoom;
      const sy = (sprite.y - cam.scrollY) * cam.zoom - 40;
      this.hintBg.setPosition(sx, sy).setVisible(true);
      this.hintText.setPosition(sx, sy).setVisible(true);
    } else {
  ...
  handleInteractInput() {
    ...
    if (!eDown) this.interactPressed = false;
    if (!this.touchInteract) this.touchInteract = false;
  }

  handleDialogueKeys() {
    const advance = this.keys.e.isDown || this.keys.space.isDown ||
                    this.keys.esc.isDown || this.touchInteract;
    if (advance && !this.interactPressed) {
      this.interactPressed = true;
      this.touchInteract = false;
      ...
      if (this.typewriterTimer && this.typewriterTimer.getProgress() < 1) {
        ...
        this.dialogueBodyText.setText(this.wrapText(line, 52));
        return;
      }
    ...
    if (!advance) this.interactPressed = false;
  }
    ...
    if (x < 192) {
      ...
      for (const [id, o] of Object.entries(OFFICES)) {
        if (x >= o.x && x <= o.x + o.w && y >= o.y && y <= o.y + o.h) {
          ...
          if (agent) roomName = `${agent.title} — ${agent.name}'s Office`;
          break;
        }
      }
    }
  ...
  openDialogue(agent) {
    ...
    this.agentDialogueIndex[agent.id] =
      (diagSetIndex + 1) % agent.dialogues.length;
  ...
  showDialogueLine(lineIndex) {
    ...
    const wrapped = this.wrapText(line, 52);

    this.dialogueBodyText.setText('');
    this.dialogueHint.setText('▼ continue');
    ...
    this.typewriterTimer = this.time.addEvent({
      ...
      callback: () => {
        ...
        if (charCount >= totalChars) {
          this.typewriterTimer = null;
          const isLast = lineIndex >= this.dialogueLines.length - 1;
          this.dialogueHint.setText(isLast ? '[ ESC / E ] Close' : '[ E ] Continue ▼');
        }
      },
      repeat: totalChars - 1,
    });
  ...
  closeDialogue() {
    ...
    this.interactPressed = true; // prevent immediate re-open
  }
  ...
  createDialogueUI() {
    ...
    const CAM_H = 600;
    const BOX_H = 170;
    const BOX_Y = CAM_H - BOX_H - 8;
    const depth = 100;

    // Main dialogue box (dark with border)
    this.dialogueBox = this.add.graphics().setScrollFactor(0).setDepth(depth);
    this.drawDialogueBox(BOX_Y, CAM_W, BOX_H);

    // Portrait (96x96, left side of box)
    this.dialoguePortrait = this.add.image(16 + 48, BOX_Y + 8 + 48, 'portrait_ace')
      .setScrollFactor(0).setDepth(depth + 1).setVisible(false).setDisplaySize(96, 96);

    // Name text
    this.dialogueNameText = this.add.text(128, BOX_Y + 10, '', {
      fontSize: '14px',
    ...
    // Title text
    this.dialogueTitleText = this.add.text(128, BOX_Y + 28, '', {
      fontSize: '10px',
      fontFamily: 'monospace',
      color: '#445566',
    }).setScrollFactor(0).setDepth(depth + 1).setVisible(false);
    ...
    this.dialogueBodyText = this.add.text(128, BOX_Y + 48, '', {
      fontSize: '12px',
      fontFamily: 'monospace',
      color: '#E8F4FF',
      wordWrap: { width: CAM_W - 148, useAdvancedWrap: false },
    }).setScrollFactor(0).setDepth(depth + 1).setVisible(false);

    // Continue hint
    this.dialogueHint = this.add.text(CAM_W - 16, BOX_Y + BOX_H - 16, '[ E ] Continue ▼', {
      fontSize: '10px',
      fontFamily: 'monospace',
      color: '#445566',
  ...
  drawDialogueBox(y, w, h) {
    ...
    g.fillRect(8, y, w - 16, h);
    // Outer border (cyan)
    g.lineStyle(2, 0x00E5FF, 0.9);
    g.strokeRect(8, y, w - 16, h);
    // Inner border (dark)
    g.lineStyle(1, 0x1A2744, 0.8);
    g.strokeRect(11, y + 3, w - 22, h - 6);
    // Portrait slot
    g.fillStyle(0x1A2744, 0.8);
    g.fillRect(16, y + 8, 96, 96);
    g.lineStyle(1, 0x00E5FF, 0.5);
    g.strokeRect(16, y + 8, 96, 96);
    // Divider line (between portrait and text)
    g.lineStyle(1, 0x1A3355, 0.6);
    g.lineBetween(120, y + 8, 120, y + h - 8);
  }
  ...
  createTouchControls() {
    ...
    const DPAD_X = 60;
    const DPAD_Y = CAM_H - 80;

    // D-pad background
    const dpadBg = this.add.graphics().setScrollFactor(0).setDepth(depth);
    dpadBg.fillStyle(0x000000, 0.3);
    dpadBg.fillCircle(DPAD_X, DPAD_Y, 70);

    const makeBtn = (x, y, label, onDown, onUp) => {
      const bg = this.add.graphics().setScrollFactor(0).setDepth(depth + 1);
      bg.fillStyle(0x1a2744, 0.7);
      bg.fillRect(x - BTN/2, y - BTN/2, BTN, BTN);
      bg.lineStyle(1, 0x00E5FF, 0.5);
      bg.strokeRect(x - BTN/2, y - BTN/2, BTN, BTN);

      const txt = this.add.text(x, y, label, {
        fontSize: '18px', fontFamily: 'monospace', color: '#00E5FF',
      }).setOrigin(0.5).setScrollFactor(0).setDepth(depth + 2);

      const zone = this.add.zone(x, y, BTN, BTN).setScrollFactor(0).setDepth(depth + 3);
      zone.setInteractive();
      zone.on('pointerdown', () => { onDown(); bg.fillStyle(0x00E5FF, 0.4); bg.fillRect(x-BTN/2, y-BTN/2, BTN, BTN); });
      zone.on('pointerup', () => { onUp(); bg.fillStyle(0x1a2744, 0.7); bg.fillRect(x-BTN/2, y-BTN/2, BTN, BTN); });
      zone.on('pointerout', () => { onUp(); bg.fillStyle(0x1a2744, 0.7); bg.fillRect(x-BTN/2, y-BTN/2, BTN, BTN); });
      return zone;
    };

    // D-pad directions
    makeBtn(DPAD_X, DPAD_Y - 52, '▲', () => this.touchDir.y = -1, () => { if (this.touchDir.y < 0) this.touchDir.y = 0; });
    makeBtn(DPAD_X, DPAD_Y + 52, '▼', () => this.touchDir.y =  1, () => { if (this.touchDir.y > 0) this.touchDir.y = 0; });
    makeBtn(DPAD_X - 52, DPAD_Y, '◄', () => this.touchDir.x = -1, () => { if (this.touchDir.x < 0) this.touchDir.x = 0; });
    makeBtn(DPAD_X + 52, DPAD_Y, '►', () => this.touchDir.x =  1, () => { if (this.touchDir.x > 0) this.touchDir.x = 0; });

    // Action button (Talk / A)
    const ACT_X = CAM_W - 60;
    const ACT_Y = CAM_H - 80;
    const actBg = this.add.graphics().setScrollFactor(0).setDepth(depth + 1);
    const drawActBtn = (pressed) => {
      actBg.clear();
      actBg.fillStyle(pressed ? 0x00E5FF : 0x1a2744, pressed ? 0.8 : 0.7);
      actBg.fillCircle(ACT_X, ACT_Y, 30);
      actBg.lineStyle(2, 0x00E5FF, 0.8);
      actBg.strokeCircle(ACT_X, ACT_Y, 30);
    };
    drawActBtn(false);

    const actLabel = this.add.text(ACT_X, ACT_Y, 'TALK', {
      fontSize: '11px', fontFamily: 'monospace', color: '#00E5FF', fontStyle: 'bold',
    }).setOrigin(0.5).setScrollFactor(0).setDepth(depth + 2);

    const actZone = this.add.zone(ACT_X, ACT_Y, 64, 64).setScrollFactor(0).setDepth(depth + 3);
    ...
    // Esc / close button (top right)
    const escBtn = this.add.text(CAM_W - 16, 8, '[ ESC ]', {
      fontSize: '10px', fontFamily: 'monospace', color: '#2d3142',
  ...
  drawWorld(g) {
    ...
    // Draw door openings (visually mark them)
    this.drawDoors(g);

    // Draw agent desk/workspace props
  ...
  drawRoom(g, room, pal) {
    ...
    g.fillRect(room.x, room.y, room.w, TILE);           // top
    g.fillRect(room.x, room.y + room.h - TILE, room.w, TILE); // bottom
    g.fillRect(room.x, room.y, TILE, room.h);           // left
    g.fillRect(room.x + room.w - TILE, room.y, TILE, room.h); // right
    // Accent line on interior of walls
    g.fillStyle(pal.accent, 0.3);
    g.fillRect(room.x + TILE, room.y + TILE, room.w - TILE * 2, 2); // inner top
    g.fillRect(room.x + TILE, room.y, 2, room.h);       // inner left
    g.fillRect(room.x + room.w - TILE - 2, room.y, 2, room.h); // inner right
  }
  ...
  drawLobby(g) {
    ...
    // Walls (top, bottom, left — right has hallway opening)
    g.fillStyle(pal.wall);
    g.fillRect(LOBBY.x, LOBBY.y, LOBBY.w, TILE);             // top
    g.fillRect(LOBBY.x, LOBBY.y + LOBBY.h - TILE, LOBBY.w, TILE); // bottom
    g.fillRect(LOBBY.x, LOBBY.y, TILE, LOBBY.h);             // left
    ...
    // Neon accent strip at floor-wall junction
    ...
    // BASeD logo on floor (large spade + text)
    const logoX = LOBBY.x + LOBBY.w / 2;
    const logoY = LOBBY.y + LOBBY.h / 2 + 80;
    g.fillStyle(pal.accent, 0.25);
    // Simple spade shape
    ...
    // Reception desk (lower portion)
    const deskY = HALLWAY.y + HALLWAY.h / 2 - 20;
    g.fillStyle(0x1A2744);
    g.fillRect(LOBBY.x + 16, deskY - 32, 128, 40);
    g.fillStyle(pal.accent, 0.8);
    g.fillRect(LOBBY.x + 16, deskY - 32, 128, 3); // top edge neon strip

    // Sign-in sheet on desk
    g.fillStyle(0xF5F0E8);
    g.fillRect(LOBBY.x + 30, deskY - 28, 32, 24);
    g.fillStyle(0x1a1a2e, 0.5);
    for (let i = 0; i < 4; i++) {
      g.fillRect(LOBBY.x + 33, deskY - 22 + i * 6, 22, 1);
    }

    // Potted plant (Gerald) upper portion of lobby
    g.fillStyle(0x1B4D3E);
    g.fillRect(LOBBY.x + 24, LOBBY.y + 24, 12, 20); // stem
    g.fillRect(LOBBY.x + 18, LOBBY.y + 24, 24, 8);  // leaves
    g.fillRect(LOBBY.x + 16, LOBBY.y + 30, 8, 6);
    g.fillRect(LOBBY.x + 32, LOBBY.y + 30, 8, 6);
    g.fillStyle(0x5C3A1E);
    g.fillRect(LOBBY.x + 22, LOBBY.y + 42, 16, 10); // pot
  ...
  drawHallway(g) {
    ...
    // Framed photos along top and bottom of hallway
    ...
    for (let i = 0; i < 8; i++) {
      ...
      g.fillStyle(accentColors[i], 0.6);
    ...
    // Water cooler (between ace and astra offices)
    ...
    g.fillStyle(0xE8F4FF, 0.4);
    g.fillRect(wcX + 6, wcY + 6, 8, 12);

    // Hallway carpet pattern hint (♠ spades — Rybo's easter egg)
    g.fillStyle(pal.floorAlt, 0.3);
    for (let i = 0; i < 8; i++) {
    ...
    // Directional signs above doorways
    const signLabels = ['ACE ▲', 'ASTRA ▲', 'DEZAYAS ▲', 'RYBO ▲', 'CHARLES ▼', 'ROMERO ▼', 'CID ▼', 'JULIUS ▼'];
    const signXs = [348, 660, 972, 1284, 348, 660, 972, 1284];
    const signYs = [HY + 4, HY + 4, HY + 4, HY + 4, HY + HH - 12, HY + HH - 12, HY + HH - 12, HY + HH - 12];
    // (text drawn via scene text objects below)
  }
  ...
  drawAceOffice(g) {
    ...
    // 3-monitor rig on north wall
    const mx = o.x + o.w / 2;
    const my = o.y + TILE + 8;
    // Desk
    g.fillStyle(0x2D3142);
    g.fillRect(o.x + 32, my + 32, o.w - 64, 20);
    g.fillStyle(pal.accent, 0.3);
    g.fillRect(o.x + 32, my + 32, o.w - 64, 2);
    // 3 monitors
    [[0, 0, 8, 10], [-52, 2, 8, 9], [52, 2, 8, 9]].forEach(([dx, dy, w, h]) => {
      g.fillStyle(0x1B2A4A);
      g.fillRect(mx + dx - w/2, my + dy, w, h);
      g.fillStyle(pal.accent, 0.4);
      g.fillRect(mx + dx - w/2 + 1, my + dy + 1, w - 2, h - 2);
      // Tiny text lines
      g.fillStyle(pal.highlight, 0.3);
      for (let i = 0; i < 4; i++) {
        g.fillRect(mx + dx - w/2 + 1, my + dy + 2 + i * 2, w - 4, 1);
      }
    });

    // Task board on right wall
    g.fillStyle(0x1B2A4A);
    g.fillRect(o.x + o.w - 64, o.y + 32, 48, 80);
    g.lineStyle(1, pal.accent, 0.4);
    g.strokeRect(o.x + o.w - 64, o.y + 32, 48, 80);
    // Color-coded task cards
    const cardColors = [0x00FF41, 0x00FF41, 0xFFE033, 0xFFE033, 0xE8192C];
    ...
    g.fillStyle(0xFFD37A, 0.08);
    g.fillTriangle(mx, my + 32, mx - 32, my + 60, mx + 32, my + 60);

    // ♠ framed ace of spades on wall
    g.fillStyle(0x2D3142);
  ...
  drawAstraOffice(g) {
    ...
    // Strategy map left wall (world map outline)
    ...
    g.fillStyle(pal.highlight, 0.4);
    g.fillRect(o.x + o.w - 72, o.y + 22, 8, 1);

    // Standing desk (taller than normal)
    g.fillStyle(0x0D1F40);
    ...
    g.lineStyle(1, pal.accent, 0.3);
    g.strokeRect(o.x + 90, o.y + 34, 14, 10);
  }
  ...
  drawDezayasOffice(g) {
    ...
    // Multi-monitor rig (centerpiece)
    const mx = o.x + o.w / 2;
    // L-shaped standing desk
    g.fillStyle(0x161B22);
    g.fillRect(o.x + 24, o.y + 32, o.w - 48, 18); // desk surface
    ...
    [[0, 0, 10, 12], [-56, 2, 8, 10], [56, 2, 8, 10]].forEach(([dx, dy, w, h]) => {
      ...
      for (let i = 0; i < 5; i++) {
        g.fillRect(mx + dx - w/2 + 1, o.y + 20 + dy + i * 2, (w - 4) * (0.4 + Math.random() * 0.5), 1);
      }
    });

    // Terminal green glow on floor
    g.fillStyle(pal.accent, 0.04);
    g.fillRect(o.x + 24, o.y + 48, o.w - 48, 32);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      g.fillStyle(pal.accent, 0.8);
      // Blinking LED (alternating)
      g.fillRect(o.x + TILE + 32, o.y + TILE + 7 + i * 12, 3, 3);
    }

    // Cable chaos on floor
    g.lineStyle(1, 0x1B3A2A, 0.8);
    ...
    const sColors = [0xFFE033, 0xFF69B4, 0x00CEC9, 0xE8192C];
    [[148, 26], [156, 20], [164, 28], [160, 20]].forEach(([dx, dy], i) => {
      g.fillStyle(sColors[i]);
      g.fillRect(o.x + dx, o.y + dy, 10, 10);
    });

    // Pour-over coffee setup corner
    g.fillStyle(0x3B1F0A);
    g.fillRect(o.x + o.w - 52, o.y + 24, 24, 20);
    g.fillStyle(0xC1440E, 0.6);
    g.fillRect(o.x + o.w - 48, o.y + 22, 16, 4);

    // WHY whiteboard
    g.fillStyle(0x1B3A2A);
    g.fillRect(o.x + o.w - 80, o.y + TILE, 64, 36);
    g.lineStyle(1, pal.accent, 0.4);
    g.strokeRect(o.x + o.w - 80, o.y + TILE, 64, 36);
    // "WHY" text represented as large marks
    g.fillStyle(pal.accent, 0.6);
    g.fillRect(o.x + o.w - 76, o.y + 24, 8, 12);
    g.fillRect(o.x + o.w - 64, o.y + 24, 8, 12);
    g.fillRect(o.x + o.w - 52, o.y + 24, 8, 12);
  ...
  drawRyboOffice(g) {
    ...
    // Worn leather couch (left side)
    g.fillStyle(0x5C3A1E);
    g.fillRect(o.x + TILE + 4, o.y + 40, 96, 32);
    g.fillStyle(0x7B4F2E);
    g.fillRect(o.x + TILE + 4, o.y + 40, 96, 8); // back cushion
    g.fillRect(o.x + TILE + 4, o.y + 48, 4, 24);  // left arm
    g.fillRect(o.x + TILE + 96, o.y + 48, 4, 24); // right arm
    // Cushion lines
    ...
    // Whiteboard (right wall)
    g.fillStyle(0xE8C99A, 0.4);
    g.fillRect(o.x + o.w - 80, o.y + TILE, 64, 48);
    g.lineStyle(1, pal.accent, 0.4);
    g.strokeRect(o.x + o.w - 80, o.y + TILE, 64, 48);
    g.fillStyle(pal.accent, 0.5);
    // "IS THIS ACTUALLY WORKING" - large scrawled text visual
    for (let i = 0; i < 4; i++) {
    ...
    const ccColors = [0xE8C99A, 0xC1440E, 0x3D2B1F];
    [[0, 0], [28, 4], [56, 0], [12, 28], [40, 24], [64, 20]].forEach(([dx, dy], i) => {
      g.fillStyle(ccColors[i % 3]);
      g.fillRect(o.x + 124 + dx, o.y + 22 + dy, 20, 14);
    });

    // Filing cabinet (special — labeled)
    ...
    for (let i = 0; i < 4; i++) {
      g.fillStyle(pal.accentColor || 0xC1440E);
  ...
  drawCharlesOffice(g) {
    ...
    [[0, 0], [60, 10]].forEach(([dx, dy]) => {
      ...
      g.lineBetween(o.x + o.w - 68 + dx, o.y + TILE + 16 + dy, o.x + o.w - 48 + dx, o.y + TILE + 16 + dy);
    });

    // Genealogy chart on back wall (partial view)
  ...
  drawRomeroStudio(g) {
    ...
    // Checkerboard floor (Britto Palace)
    ...
    for (let i = 0; i < 12; i++) {
      ...
      // Britto-style geometric fill
      g.fillStyle(artColors[(i + 3) % artColors.length], 0.5);
    ...
    // Paint supplies scattered on floor
    const pColors = [0xE8192C, 0x0047AB, 0xFFD700, 0x2ECC40, 0xFF69B4];
    pColors.forEach((c, i) => {
      g.fillStyle(c);
      g.fillRect(o.x + 140 + i * 16, BOT - 80, 10, 4); // paint tube
      g.fillRect(o.x + 140 + i * 16 + 8, BOT - 80 + 2, 2, 2); // cap
    });
    ...
    for (let b = 0; b < 4; b++) {
      ...
      g.fillStyle(pColors[b]);
      g.fillRect(o.x + 230 + b * 3, BOT - 96, 2, 4);
    }

    // Mood board
    g.fillStyle(0xA0522D, 0.5);
    g.fillRect(o.x + o.w - 96, o.y + TILE + 4, 80, 80);
    g.lineStyle(1, 0x1c1c2e, 0.6);
    ...
    // Paint splatters on floor (accidental pixels)
    [[120, 20, 0xFFD700], [200, 40, 0x0047AB], [80, 60, 0xFF69B4], [260, 10, 0x2ECC40]].forEach(([dx, dy, c]) => {
      g.fillStyle(c, 0.7);
      g.fillRect(o.x + dx, BOT - 72 + dy, 3, 3);
      g.fillRect(o.x + dx + 4, BOT - 70 + dy, 2, 2);
    });
  }
  ...
  drawCidArcade(g) {
    ...
    // Tiny game on screen
    g.fillStyle(0xffffff, 0.8);
    g.fillRect(o.x + o.w - 58, o.y + TILE + 24, 4, 8);  // player
    g.fillStyle(0xE8192C, 0.8);
    g.fillRect(o.x + o.w - 48, o.y + TILE + 18, 6, 4);  // enemy
    ...
    // Game design whiteboard (left wall)
    g.fillStyle(0x1a0a2e);
    g.fillRect(o.x + TILE, o.y + TILE, 72, 48);
    g.lineStyle(1, pal.accent, 0.5);
    g.strokeRect(o.x + TILE, o.y + TILE, 72, 48);
    // Level layout sketch (platform lines)
    g.lineStyle(1, pal.accent, 0.7);
    ...
    // Enemy circles
    g.lineStyle(1, pal.highlight, 0.5);
    g.strokeCircle(o.x + 44, o.y + 50, 4);
    g.strokeCircle(o.x + 62, o.y + 40, 4);

    // Kanban board (main feature wall)
    g.fillStyle(0x1a0a2e);
    ...
    [[104, 32, 0xFFE033], [104, 48, 0xFFE033], [158, 32, pal.accent], [210, 32, pal.highlight], [210, 48, pal.highlight]].forEach(([dx, dy, c]) => {
      g.fillStyle(c, 0.7);
      g.fillRect(o.x + dx, o.y + dy, 40, 10);
    });

    // Cartridge shelf (back wall)
    g.fillStyle(0x2D1B4E);
    g.fillRect(o.x + TILE, o.y + 72, o.w - TILE * 2 - 72, 16);
    g.lineStyle(1, pal.accent, 0.3);
    g.strokeRect(o.x + TILE, o.y + 72, o.w - TILE * 2 - 72, 16);
    const cartColors = [0x7B2D8B, 0xE8192C, 0x0047AB, 0x2ECC40, 0xFFD700, 0x00CEC9, 0xE8192C, 0x7B2D8B, 0xFFD700, 0x2ECC40];
    cartColors.forEach((c, i) => {
      g.fillStyle(c);
      g.fillRect(o.x + TILE + 4 + i * 22, o.y + 74, 18, 12);
      g.lineStyle(1, 0x1c1c2e, 0.5);
      g.strokeRect(o.x + TILE + 4 + i * 22, o.y + 74, 18, 12);
    });

    // d20 on desk
    // d20 — draw outline triangle then inner lighter triangle
    g.fillStyle(pal.accent, 0.7);
    ...
    g.fillStyle(0x7B2D8B, 0.3);
    g.fillEllipse(o.x + 200, BOT - 76, 40, 18);

    // Earthbound paused on monitor
    g.fillStyle(0x0D1117);
    ...
    // Tiny Earthbound scene (green grass + character)
    g.fillStyle(0x1a8c1a, 0.7);
    g.fillRect(o.x + 118, BOT - 144, 76, 12);
    g.fillStyle(0xffffff, 0.9);
    g.fillRect(o.x + 148, BOT - 148, 6, 10);  // Ness
    g.fillStyle(0x0000ff, 0.8);
    g.fillRect(o.x + 148, BOT - 152, 6, 5);   // hat
  }
  ...
  drawJuliusOffice(g) {
    ...
    // Bridge silhouette
    g.fillStyle(pal.highlight, 0.3);
    g.fillRect(o.x + TILE + 20, o.y + TILE + 40, 200, 24); // water
    g.fillStyle(0x1E3C34, 0.7);
    g.fillRect(o.x + TILE + 30, o.y + TILE + 28, 180, 14); // bridge deck
    // Bridge towers
    g.fillStyle(0x2A5040, 0.8);
    ...
    g.fillStyle(0x2ECC40, 0.6);
    g.fillRect(o.x + o.w - 62, o.y + TILE + 54, 12, 14); // continent
  ...
  drawDoors(g) {
    // Draw door openings in walls — clear the wall color and draw a darker floor strip
    Object.entries(OFFICES).forEach(([id, o]) => {
      ...
      if (o.isTop) {
        // Door in bottom wall (y = o.y + o.h - TILE)
  ...
  drawAgentDesks(g) {
    // Draw desk/workspace for each agent at their position
    Object.values(AGENTS_DATA).forEach(agent => {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // ROOM TEXT LABELS (called after drawWorld)
  // These use Phaser text objects placed in the world
  // ──────────────────────────────────────────────────────────────────────────
  addRoomLabels() {
    const style = {
      fontSize: '10px', fontFamily: 'monospace', color: '#445566', alpha: 0.5
    };
    // Add signs above office doors
    [
      { x: 348, y: HALLWAY.y + 4, text: 'ACE ▲' },
      { x: 660, y: HALLWAY.y + 4, text: 'ASTRA ▲' },
      { x: 972, y: HALLWAY.y + 4, text: 'DEZAYAS ▲' },
      { x: 1284, y: HALLWAY.y + 4, text: 'RYBO ▲' },
      ...
      this.add.text(x, y, text, { ...style, color: '#2D3E55' }).setOrigin(0.5, 0).setDepth(5);
    });

    // Lobby label
    this.add.text(96, LOBBY.y + LOBBY.h / 2 - 100, 'BASeD HQ', {
      fontSize: '12px', fontFamily: 'monospace', color: '#00E5FF', alpha: 0.4,
    }).setOrigin(0.5).setDepth(5);

    // "Gerald" label on plant
    this.add.text(LOBBY.x + 36, LOBBY.y + 56, 'Gerald', {
